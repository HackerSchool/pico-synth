...
lookup_table_interpolate(const std::array<int16_t, WAVE_TABLE_LEN> &table,
                         int16_t x, size_t q) {
    int16_t index =
        ((int32_t)x * (WAVE_TABLE_LEN )) >> q; // Scale based on q
    // printf("index %d\n", index);
...
int main() {
    ...
    for (i = 0; i < m; ++i) {
        ...
        // Jesus Christ this is so shit...
        // But we don't need to multiply by 2Pi since our wave table is sampled by x*2*pi
        int16_t z_su = ((int32_t)(2 * i + 1) * (1 << 13) / n);
        // printf("i = %d z_su = %f\n", z_su, q1_15_to_float(z_su));
        // printf("i = %d z_cu = %f\n", i, q1_15_to_float(z_cu));
        b = (int32_t)lookup_table_interpolate(sine_wave_table, z_su, 15) * su >>
            14;
        //
        // b = ;
        // printf("i = %d b = %f\n", i, q1_15_to_float(b));
        c = (int32_t)lookup_table_interpolate(cos_wave_table, z_su, 15) * cu >>
            14;
        ...
        d1[i] = 2.0 * (((1 << 15) - a2_c)<<13) / s;
        d2[i] = (-(a2_c - ab_2 + (1 << 15)) << 14) / s;
        printf("i = %d, A[i] = %lf, d1[i] = %lf, d2[i] = %lf\n", i, q1_15_to_float(A[i]), q2_14_to_float(d1[i]),
               q1_15_to_float(d2[i]));
    }
